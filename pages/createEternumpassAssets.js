...
const EternumPassAddress = process.env.NEXT_PUBLIC_ETERNUMPASS_ADDRESS;
export default function CreateEternumpassNft() {
  ...
  const [contractEternumpassAddress, setContarctEternumpassAddress] = useState([]);
  ...
  const buyTokens = useContract({
    addressOrName: EternumPassAddress,
    contractInterface: EternumPass.abi,
  ...
  const getAllContarctData = () => {
    ...
    axios
      ...
      .then(async (response) => {
        if (response?.data?.contractName ==='EternumPass') {
            setContarctEternumpassAddress(response.data.contractAddress)
        }
      })
      .catch((error) => {
  ...
  async function createItem() {
        try {
          const tx = await buyTokens.subscribe();
          tx.wait().then(async (transaction) => {
            console.log('response while eternumpass nft creation',transaction)
          });
        } catch (error) {
            console.log('error while eternumpass nft creation',error)
        } 
