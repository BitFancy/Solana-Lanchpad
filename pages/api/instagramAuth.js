...
export default async function handler(req, res) {
  ...
  try {
    ...
    formData.append(
      "client_id",
      process.env.NEXT_PUBLIC_MYRIADFLOW_INSTAGRAM_CLIENT_ID
    );
    formData.append(
      "client_secret",
      process.env.NEXT_PUBLIC_MYRIADFLOW_INSTAGRAM_CLIENT_SECRET
    );
    ...
    try {
      ...
      res.setHeader(
        "Set-Cookie",
        `instaData=${JSON.stringify(
          userData.data
        )}; Path=/; Expires=${farFutureDate.toUTCString()};`
      );
