...
import { getAllInstagenNfts } from "./api/instagenAssets";
function GetAllInstagenNft(props) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      getallInstagenAssets();
    }
  }, []);
  const getallInstagenAssets = async () => {
    try {
      const endPoint = props?.router?.query?.redirectURL?.slice(
        0,
        props?.router?.query?.redirectURL?.indexOf("/graphql")
      );
      const { InstagenAssetCreateds } = await getAllInstagenNfts({
        endPoint: endPoint,
      });
      setAsseetsData(InstagenAssetCreateds);
  ...
  return (
    <LayoutDashbord
      title="Instagen NFts"
      description="Used to Show All Instagen NFTs Details"
    >
      <div>
